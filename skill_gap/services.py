"""
services.py
===========
All helper functions:
- CSV helpers (sectors, occupations)  [kept for the /occupations listing]
- Tracker authentication
- Skill fetching from SERVICE2_URL
- Distinct skill pool across multiple occupations
- Demand (jobs) & Supply (CVs) count fetching
- Gap analysis logic (rank score, gap score)
- Curriculum cross-check (YES/NO + in which courses)
"""
import os
import time
import logging
import requests
import pandas as pd
import urllib3
import mysql.connector
from typing import List, Dict, Optional
from collections import defaultdict
from concurrent.futures import ThreadPoolExecutor, as_completed

logger = logging.getLogger(__name__)
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

SERVICE2_URL = os.getenv("REQUIRED_SKILLS_SERVICE_URL", "https://portal.skillab-project.eu/diversity-analysis")
TRACKER_BASE_URL = os.getenv("API_TRACKER_BASE_URL", "https://skillab-tracker.csd.auth.gr/api")

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
CSV_PATH = os.path.join(BASE_DIR, "data", "New_occupation_table.csv")

# Curriculum DB (same database used by the policy analysis)
DB_CONFIG = {
    "host": os.getenv("DB_HOST", "mysql-curriculum-skill"),
    "port": int(os.getenv("DB_PORT", 3306)),
    "user": os.getenv("DB_USER", "root"),
    "password": os.getenv("DB_PASSWORD", "root"),
    "database": os.getenv("DB_NAME", "skillcrawl"),
}


# ==========================================
# TRACKER AUTH (cached token)
# ==========================================
_token_cache = {"token": None, "expires_at": 0}


def get_tracker_token() -> str:
    """
    Returns a cached token if not expired.
    Logs in only if there is no token or it has expired (50-minute cache).
    """
    now = time.time()
    if _token_cache["token"] and now < _token_cache["expires_at"]:
        logger.info("Using cached tracker token.")
        return _token_cache["token"]

    login_url = f"{TRACKER_BASE_URL}/login"
    payload = {
        "username": os.getenv("TRACKER_USERNAME"),
        "password": os.getenv("TRACKER_PASSWORD")
    }
    headers = {"Content-Type": "application/json", "accept": "application/json"}
    logger.info(f"Attempting tracker login for user: {os.getenv('TRACKER_USERNAME')}")
    try:
        response = requests.post(
            login_url, json=payload, headers=headers,
            verify=False, timeout=120
        )
        response.raise_for_status()
        token = response.text.strip().strip('"')
        _token_cache["token"] = token
        _token_cache["expires_at"] = now + (50 * 60)
        logger.info("✅ Tracker login successful. Token cached for 50 minutes.")
        return token
    except Exception as e:
        logger.error(f"❌ Tracker login failed: {e}")
        return ""


# ==========================================
# CSV HELPERS (kept only for listing available occupations)
# ==========================================
def _read_csv() -> Optional[pd.DataFrame]:
    if not os.path.exists(CSV_PATH):
        logger.error(f"CSV not found at: {CSV_PATH}")
        return None
    try:
        try:
            df = pd.read_csv(CSV_PATH, sep=None, engine='python', on_bad_lines='skip', encoding='utf-8')
        except Exception:
            df = pd.read_csv(CSV_PATH, sep=None, engine='python', on_bad_lines='skip', encoding='latin-1')
        df.columns = df.columns.str.strip().str.replace('"', '')
        return df
    except Exception as e:
        logger.error(f"Error reading CSV: {e}")
        return None


def load_sectors() -> List[str]:
    df = _read_csv()
    if df is None or 'Label3' not in df.columns:
        return []
    return sorted(df['Label3'].dropna().astype(str).str.strip().str.strip('"').unique().tolist())


def load_occupations(sector_filter: Optional[str] = None) -> List[str]:
    df = _read_csv()
    if df is None:
        return []
    if sector_filter and 'Label3' in df.columns:
        df = df[df['Label3'].astype(str).str.contains(sector_filter, case=False, na=False)]

    def clean(text):
        if not isinstance(text, str):
            return ""
        return text.replace("β€™", "'").replace("â€™", "'").strip().strip('"').strip("'")

    occupations = set()
    if 'Label4' in df.columns:
        occupations.update(df['Label4'].dropna().apply(clean).tolist())
    if 'Label3' in df.columns:
        occupations.update(df['Label3'].dropna().apply(clean).tolist())
    result = [o for o in occupations if o and len(o) > 2]
    logger.info(f"✅ Loaded {len(result)} occupations")
    return result


# ==========================================
# SKILL FETCHING FROM SERVICE2 (Trig)
# ==========================================
def _fetch_skills_for_occupation(occupation: str, min_val: float) -> tuple:
    """
    Fetch required skills for a single occupation from Trig.
    Trig item: {"Role":..., "Skill":<label>, "Importance":<0..1>, "SkillId":<esco url>}
    Returns (occupation, [ {skill_name, skill_id, value} ]).
    """
    try:
        resp = requests.post(
            f"{SERVICE2_URL}/required_skills_service",
            json={"occupation_name": occupation},
            timeout=240
        )
        if resp.status_code == 200 and resp.text:
            data = resp.json()
            # Error string inside a list on failure
            if isinstance(data, list) and data and isinstance(data[0], str):
                logger.warning(f"[{occupation}] Trig error: {data[0]}")
                return occupation, []

            skills = []
            if isinstance(data, list):
                for item in data:
                    if isinstance(item, dict):
                        val = item.get('Importance', 0) or 0   # FIXED: was 'Value'
                        if val >= min_val:
                            sid = item.get("SkillId") or item.get("skill_id") or item.get("Id")
                            skills.append({
                                "skill_name": (item.get("Skill") or "").strip(),
                                "skill_id": sid.strip() if sid else None,
                                "value": val
                            })
            return occupation, skills
        return occupation, []
    except Exception as e:
        logger.warning(f"Failed skills for '{occupation}': {e}")
        return occupation, []


def fetch_all_skills_parallel(occupations: List[str], min_val: float = 0.0, max_workers: int = 3) -> Dict[str, List]:
    results = {}
    total = len(occupations)
    logger.info(f"Fetching skills for {total} occupations (max_workers={max_workers})...")
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futures = {ex.submit(_fetch_skills_for_occupation, occ, min_val): occ for occ in occupations}
        done = 0
        for future in as_completed(futures):
            occ, skills = future.result()
            if skills:
                results[occ] = skills
            done += 1
            if done % 10 == 0:
                logger.info(f"  Skills progress: {done}/{total}")
    logger.info(f"✅ Skills fetched for {len(results)}/{total} occupations with results.")
    return results


def build_distinct_skill_pool(
    occ_skills_map: Dict[str, List[Dict]],
    top_n: Optional[int] = None
) -> List[Dict]:
    """
    Merge skills across all occupations into a DISTINCT pool keyed by skill_id
    (ESCO url). For each unique skill, remember which occupations required it
    and keep the highest importance value seen.

    If top_n is set, keep only the top_n skills by importance across the whole
    pool (single unified ranking basis).

    Returns: [ {skill_id, skill_name, value, occupations: [...]}, ... ]
    """
    pool: Dict[str, Dict] = {}
    for occ, skills in occ_skills_map.items():
        for s in skills:
            sid = s.get("skill_id")
            if not sid:
                continue
            if sid not in pool:
                pool[sid] = {
                    "skill_id": sid,
                    "skill_name": s.get("skill_name"),
                    "value": s.get("value", 0) or 0,
                    "occupations": set()
                }
            pool[sid]["occupations"].add(occ)
            # keep the highest importance seen for this skill
            if (s.get("value", 0) or 0) > pool[sid]["value"]:
                pool[sid]["value"] = s.get("value", 0) or 0

    merged = list(pool.values())
    merged.sort(key=lambda x: x["value"], reverse=True)
    if top_n is not None:
        merged = merged[:top_n]

    # convert occupation sets to sorted lists for JSON serialization
    for m in merged:
        m["occupations"] = sorted(m["occupations"])

    logger.info(f"Distinct skill pool: {len(merged)} unique skills "
                f"(top_n={top_n}) across {len(occ_skills_map)} occupations.")
    return merged


# ==========================================
# TRACKER API: DEMAND (jobs) & SUPPLY (CVs)
# ==========================================
def _get_skill_job_count(skill_id: str, token: str) -> int:
    try:
        headers = {
            "Authorization": f"Bearer {token}",
            "accept": "application/json",
            "Content-Type": "application/x-www-form-urlencoded"
        }
        data = f"skill_ids={requests.utils.quote(skill_id)}&skill_ids_logic=or"
        resp = requests.post(
            f"{TRACKER_BASE_URL}/jobs?page=1&page_size=1",
            headers=headers, data=data, verify=False, timeout=240
        )
        if resp.ok:
            return resp.json().get("count", 0)
        logger.warning(f"  Job count failed {resp.status_code}: {resp.text[:80]}")
        return 0
    except Exception as e:
        logger.warning(f"  _get_skill_job_count error: {e}")
        return 0


def _get_skill_profile_count(skill_id: str, token: str) -> int:
    try:
        headers = {
            "Authorization": f"Bearer {token}",
            "accept": "application/json",
            "Content-Type": "application/x-www-form-urlencoded"
        }
        data = f"skill_ids={requests.utils.quote(skill_id)}&skill_ids_logic=or"
        resp = requests.post(
            f"{TRACKER_BASE_URL}/profiles?page=1&page_size=1",
            headers=headers, data=data, verify=False, timeout=240
        )
        if resp.ok:
            return resp.json().get("count", 0)
        logger.warning(f"  Profile count failed {resp.status_code}: {resp.text[:80]}")
        return 0
    except Exception as e:
        logger.warning(f"  _get_skill_profile_count error: {e}")
        return 0


def fetch_counts_parallel(skills: List[Dict], token: str, max_workers: int = 10) -> List[Dict]:
    """Parallel fetch of demand + supply counts for all skills."""
    logger.info(f"  Fetching counts for {len(skills)} skills (max_workers={max_workers})...")

    def fetch_one(skill: Dict) -> Dict:
        sid = skill.get("skill_id")
        if not sid:
            logger.warning(f"  Skill '{skill.get('skill_name')}' has no skill_id — skipping")
            return {**skill, "demand_count": 0, "supply_count": 0}
        demand = _get_skill_job_count(sid, token)
        supply = _get_skill_profile_count(sid, token)
        return {**skill, "demand_count": demand, "supply_count": supply}

    enriched = []
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futures = [ex.submit(fetch_one, s) for s in skills]
        for f in as_completed(futures):
            enriched.append(f.result())

    for s in enriched[:3]:
        logger.info(f"  Sample: {s.get('skill_name')} | demand={s.get('demand_count')} | supply={s.get('supply_count')}")
    return enriched


# ==========================================
# GAP ANALYSIS LOGIC
# ==========================================
def compute_rank_score(skills: List[Dict], count_key: str) -> List[Dict]:
    """
    Only skills with count > 0 get a rank score.
    Skills with count = 0 get score = 0.0
    Formula: ((N - position) + 1) / N * 100  (top of the list = 100%)
    """
    score_key = f"{count_key}_score"
    with_count = [s for s in skills if (s.get(count_key) or 0) > 0]
    without_count = [s for s in skills if (s.get(count_key) or 0) == 0]

    ranked = sorted(with_count, key=lambda x: x.get(count_key, 0), reverse=True)
    N = len(ranked)

    for i, skill in enumerate(ranked):
        skill[score_key] = round(((N - i) / N) * 100, 2) if N > 0 else 0.0
    for skill in without_count:
        skill[score_key] = 0.0

    return ranked + without_count


def compute_gap(demand_skills: List[Dict], supply_skills: List[Dict]) -> List[Dict]:
    """
    gap_score = demand_score - supply_score
    +100% = hot skill (in demand but not supplied)
    -100% = oversupplied (supplied but not in demand)
    Carries through occupations (which occupations required each skill).
    """
    demand_map = {s.get("skill_id"): s for s in demand_skills if s.get("skill_id")}
    supply_map = {s.get("skill_id"): s for s in supply_skills if s.get("skill_id")}
    all_ids = set(demand_map.keys()) | set(supply_map.keys())

    gap_results = []
    for sid in all_ids:
        d = demand_map.get(sid, {})
        s = supply_map.get(sid, {})
        d_score = d.get("demand_count_score", 0.0)
        s_score = s.get("supply_count_score", 0.0)
        gap = round(d_score - s_score, 2)
        gap_results.append({
            "skill_id": sid,
            "skill_name": d.get("skill_name") or s.get("skill_name"),
            "occupations": d.get("occupations") or s.get("occupations") or [],
            "demand_count": d.get("demand_count", 0),
            "supply_count": s.get("supply_count", 0),
            "demand_score": d_score,
            "supply_score": s_score,
            "gap_score": gap
        })
    return sorted(gap_results, key=lambda x: x["gap_score"], reverse=True)


# ==========================================
# CURRICULUM CROSS-CHECK (YES/NO + in which courses)
# ==========================================
def check_skills_in_curriculum(skill_ids: List[str]) -> Dict[str, Dict]:
    """
    For each skill_id (ESCO url), checks whether it is taught anywhere in the
    universities DB and in which courses.

    Returns: { skill_id: {"in_curriculum": bool, "courses": [ "<course> (<university>) - [<country>]", ... ]} }
    Matching is done on Skill.skill_url (robust, label-independent).
    """
    result = {sid: {"in_curriculum": False, "courses": []} for sid in skill_ids if sid}
    if not skill_ids:
        return result

    conn = None
    BATCH_SIZE = 50
    try:
        conn = mysql.connector.connect(**DB_CONFIG)
        cursor = conn.cursor(dictionary=True)

        ids = [sid for sid in skill_ids if sid]
        for i in range(0, len(ids), BATCH_SIZE):
            batch = ids[i:i + BATCH_SIZE]
            placeholders = ', '.join(['%s'] * len(batch))
            query = f"""
                SELECT s.skill_url, c.lesson_name, u.university_name, u.country
                FROM Skill s
                JOIN CourseSkill cs ON s.skill_id = cs.skill_id
                JOIN Course c ON cs.course_id = c.course_id
                JOIN University u ON c.university_id = u.university_id
                WHERE s.skill_url IN ({placeholders})
                LIMIT 2000
            """
            cursor.execute(query, batch)
            rows = cursor.fetchall()
            for r in rows:
                sid = r["skill_url"].strip() if r.get("skill_url") else None
                if not sid or sid not in result:
                    continue
                result[sid]["in_curriculum"] = True
                entry = f"{r['lesson_name']} ({r['university_name']}) - [{r['country']}]"
                if entry not in result[sid]["courses"]:
                    result[sid]["courses"].append(entry)

    except Exception as e:
        logger.error(f"DB Error in check_skills_in_curriculum: {e}")
    finally:
        if conn and conn.is_connected():
            conn.close()

    return result